...
- **2026-08-24** The frontend uses **Zod** to parse every API response and SignalR payload at the boundary (user requirement, 2026-08-24): the money-as-string and snake_case-status contracts are validated at runtime instead of trusted; backend validation remains FluentValidation per the pinned stack.
- **2026-08-24** Zod id fields use `z.guid()`, not `z.uuid()`: Zod 4's `uuid()` enforces RFC-4122 version/variant bits, which .NET GUIDs (and our fixed seed ids like `1111…`) legitimately do not satisfy.
- **2026-08-24** Frontend is Angular 22 (standalone, signals, zoneless — the CLI default) on Node 24 LTS via nvm; the CLI refused the system Node 24.12.
- **2026-08-24** A malformed SignalR payload is dropped and triggers a full re-sync rather than a partial patch — same recovery path as a rejected transition.
